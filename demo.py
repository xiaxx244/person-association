...
'''
    outPath = "etc/"
    image_dir = "pool_image/"
    text_dir="pool_pose/"
    TEST_IMAGE_PATHS = glob.glob(os.path.join(image_dir, '*.jpg'))
    TEST_IMAGE_PATH=natsort.natsorted(TEST_IMAGE_PATHS, reverse=False)
    text_dir=natsort.natsorted(os.listdir(text_dir), reverse=False)
    i=0
    viz=True
    while i <len(TEST_IMAGE_PATHS)-1:
        imname1=TEST_IMAGE_PATHS[i]
        imname2=TEST_IMAGE_PATHS[i+1]
        posename1=text_dir[i]
        posename2=text_dir[i+1]
        im1, im2, humans1, humans2, ids1, ids2 = associate_pairs(imname1, posename1, imname2, posename2)
        print ("The pairs are {0}".format(zip(ids1, ids2)))
        ...
        if viz:
            ...
            im_an2 = annotatePoses(im2, humans2, ids2, pids)
            combo = np.concatenate((im_an1, im_an2), axis=1)
            cv2.imwrite(outPath+str(i)+".png",combo)
            #cv2.imshow("test", combo)
            #cv2.waitKey(2000)
        i=i+1
